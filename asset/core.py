...
class Asset(object):
    ...
    def __create_server_info(self, ignore_errs=False):
        '''
        self.clean_data:就是待审批区表中的data字段->asset的所有数据
        :param ignore_errs:
        :return:
        '''
        try:
    ...
    def __update_asset_component(self, data_source, fk, update_fields, identify_field=None):
        ...
        try:
            component_obj = getattr(self.asset_obj, fk) #nic_set -> nic_obj
            if hasattr(component_obj, 'select_related'):  # this component is reverse m2m relation with Asset model
                objects_from_db = component_obj.select_related() # 所有的nic models对象
                for obj in objects_from_db:
                    key_field_data = getattr(obj, identify_field) #拿到唯一值的值
                    ...
                    if type(data_source) is list:
                        for source_data_item in data_source: #到客户端的数据 源里去找到跟 服务器数据库中key_field_data对应的条目
                            key_field_data_from_source_data = source_data_item.get(identify_field) #获取到客户端唯一值的值
                            if key_field_data_from_source_data: #如果客户端的这个值不为空
    ...
    def __compare_componet(self, model_obj, fields_from_db, data_source):
        """
        :param model_obj: 数据库里面的这条网卡记录对象
        :param fields_from_db: 客户端与数据库对比的字段
        :param data_source:  客户端数据源
        :return: 修改本条网卡记录
        """
        print('---going to compare:[%s]' % model_obj, fields_from_db)
        print('---source data:', data_source)
        for field in fields_from_db: #name sn model macaddr ipaddr
            #统一数据类型，以数据里面的字段类型为准
            #去出双方的字段值
            #3.对比，根据对比结果，确定是否更新数据库
            val_from_db = getattr(model_obj, field) # 获取到数据库中对应字段的值
            val_from_data_source = data_source.get(field) #获取客户端汇报过来的字段值
            if val_from_data_source: #如果客户端汇报的过来的值不为空，开始统一字段类型
                ...
                if val_from_db == val_from_data_source:  # 如果字段相等不做任何处理
                    pass
